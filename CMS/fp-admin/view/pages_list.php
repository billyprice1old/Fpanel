  ...
  <table class="hover">
       ...
       echo'
        ...
        <td >'.$result[$i]['PostTitle'].'<br/>';
            if($result[$i]['PostAuthor'] == $_SESSION['infos']['id']){
                echo '<a href="pages_add.php?id='.$result[$i]['PostId'].'" class="small">Edit </a> |
                <a href="#" onclick="msgbox('."'Do you want to delete this page?','?delid=".$result[$i]['PostId']."','_self','yesno'".')" class="small">Delete</a>
                ';
            }
            if($_SESSION['infos']['role']== 'Admin'){
                echo ' | <a href="posts_preview.php?id='.$result[$i]['PostId'].'" target="_blank" class="small">View</a>
                '.$quick_pub;
            }
